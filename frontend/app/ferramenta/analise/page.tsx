...
export default function AnalisePage() {
  ...
  useEffect(() => {
    ...
    const storedMostrarOpcoesAvancadas = localStorage.getItem('mostrarOpcoesAvancadas') === 'true';
    ...
    if (storedMostrarOpcoesAvancadas) {
      const storedUsarDataset1 = localStorage.getItem('usarDataset1') === 'true';
      const storedUsarDataset2 = localStorage.getItem('usarDataset2') === 'true';

      setUsarDataset1(storedUsarDataset1);
      setUsarDataset2(storedUsarDataset2);

      if (storedUsarDataset1) {
        const storedModelDataset1 = localStorage.getItem('selectedModelDataset1');
        if (storedModelDataset1) setSelectedModelDataset1(storedModelDataset1);
      }

      if (storedUsarDataset2) {
        const storedModelDataset2 = localStorage.getItem('selectedModelDataset2');
    ...
    return (
        <div className="container mx-auto py-8 px-4">
          <div className="text-center space-y-4">
            <h2 className="text-2xl font-bold text-gray-800">Nenhuma imagem analisada</h2>
            <p className="text-gray-600">Você precisa analisar uma imagem primeiro.</p>
  ...
  return (
      <div className="container mx-auto py-8 px-4">
        <div className="mb-6 flex justify-between items-center">
          <Link href="/ferramenta" className="flex items-center text-blue-600 hover:text-blue-800">
          ...
          <div className="text-sm text-gray-500">
            {mostrarOpcoesAvancadas ? (
              ...
              Modo: <span className="font-medium">Avançado</span> |
                  {usarDataset1 && <span> Dataset 1: <span className="font-medium">{selectedModelDataset1}</span></span>}
                  {usarDataset1 && usarDataset2 && <span> | </span>}
                  {usarDataset2 && <span> Dataset 2: <span className="font-medium">{selectedModelDataset2}</span></span>}
            </span>
            ) : (
                <span>Modelo: <span className="font-medium">{selectedModel}</span></span>
            )}
          </div>
        </div>
