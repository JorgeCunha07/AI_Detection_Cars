...
# Verifica e configura o pad_token
...
# Função de geração de resposta (não adiciona o turno do usuário, assume que já foi adicionado no histórico)
def generate_response(historico_conversa, max_new_tokens=200, temperature=0.7, top_p=0.9):
    ...
    with torch.no_grad():
        outputs = model.generate(
            ...
            top_p=top_p,
            do_sample=False,  # Para teste, usando decodificação greedy
    ...
    resposta = response[len(prompt):].strip()
    
    #if not resposta or len(resposta.split()) < 3:
        #resposta = "Olá! Sou o seu assistente. Como posso ajudar?"
    
    historico_conversa.append(f"<ASSISTANT>: {resposta}")
    # Limita o histórico para os últimos 12 turnos
    if len(historico_conversa) > 12:
        historico_conversa = historico_conversa[-12:]
    
    return resposta, historico_conversa

# Funções para carregar dados de quiz e Código da Estrada (para outros modos de operação)
def load_quiz_data():
    try:
...
def main():
    ...
    print(f" Digite '{Fore.YELLOW}quiz{Style.RESET_ALL}{Fore.CYAN}' para iniciar o modo de perguntas e respostas")
    print(f" Digite '{Fore.YELLOW}artigo X{Style.RESET_ALL}{Fore.CYAN}' para consultar um artigo específico")
    print(f" Digite '{Fore.YELLOW}ajuda{Style.RESET_ALL}{Fore.CYAN}' para ver os comandos disponíveis")
    print(f" Digite '{Fore.YELLOW}sair{Style.RESET_ALL}{Fore.CYAN}' para terminar o chat")
    print(f" Digite '{Fore.YELLOW}conversa{Style.RESET_ALL}{Fore.CYAN}' para iniciar uma conversa natural")
    ...
    while True:
        ...
        if modo_quiz and not pergunta_atual:
            ...
            if pergunta_atual:
                ...
                print(f"{Fore.YELLOW}(Digite 'dica' para ver a resposta correta ou 'encerrar quiz' para sair do modo quiz){Style.RESET_ALL}")
            else:
        ...
        if user_input.lower() == "sair":
            ...
            print(f"- {Fore.YELLOW}quiz{Style.RESET_ALL}: Inicia o modo de perguntas e respostas")
            print(f"- {Fore.YELLOW}artigo X{Style.RESET_ALL}: Consulta um artigo específico do Código da Estrada")
            print(f"- {Fore.YELLOW}conversa{Style.RESET_ALL}: Inicia uma conversa natural")
            ...
            pergunta_atual = None
            historico_conversa = []
            print(f"{Fore.GREEN}Entrando no modo QUIZ. Vou testar os seus conhecimentos!{Style.RESET_ALL}")
            continue
        elif user_input.lower() == "conversa":
            modo_quiz = False
            modo_conversa = True
            pergunta_atual = None
            # Define um início para a conversa natural
            historico_conversa = ["<ASSISTANT>: Olá! Podemos conversar sobre qualquer assunto. Como posso ajudar?"]
            print(f"{Fore.GREEN}Entrando no modo CONVERSA. Vamos conversar!{Style.RESET_ALL}")
            continue
            ...
            try:
                ...
                for artigo in codigo.get("articles", []):
                    if artigo_num in artigo.get("article", "").lower():
                        ...
                        break
                if not encontrado:
                    print(f"{Fore.RED}Artigo não encontrado. Tente especificar melhor (ex: 'artigo 1.º'){Style.RESET_ALL}")
            except Exception:
                print(f"{Fore.RED}Formato inválido. Use 'artigo X' (ex: 'artigo 1.º'){Style.RESET_ALL}")
            continue
        
        # Se estiver em modo QUIZ
        if modo_quiz and pergunta_atual:
            ...
            # Modo CONVERSA (ou outros modos não-QA)
            # Adiciona o turno do usuário apenas uma vez
            historico_conversa.append(f"<USER>: {user_input}")
            resposta, historico_conversa = generate_response(historico_conversa)
